...
export interface CompositeOptions {
  frames: string[];
  filter: FilterKey;
  frameFilters?: FilterKey[]; // per-frame overrides; falls back to filter if absent
  brightness: number;
  contrast: number;
  themeColor: string;
  caption: string;
...
function drawTextLayer(
  ...
  const y          = framesAreaTop + (framesAreaHeight * layer.y) / 100;
...
async function drawSticker(
  ...
  if (def.type === "image") {
    ...
    drawImageStickerOnCanvas(ctx, sticker, img, canvasWidth, framesAreaTop, framesAreaHeight);
  } else {
    drawTextSticker(ctx, sticker, def, canvasWidth, framesAreaTop, framesAreaHeight);
  }
  ctx.restore();
}
  ...
  for (const ref of stack) {
    if (ref.kind === "sticker") {
      ...
      if (sticker && sticker.visible)
        await drawSticker(ctx, sticker, canvas.width, canvas.height, framesAreaTop, framesAreaHeight);
    } else {
      const layer = textLayers.find((t) => t.id === ref.id);
      if (layer && layer.visible)
        drawTextLayer(ctx, layer, canvas.width, canvas.height, framesAreaTop, framesAreaHeight);
    }
  }
