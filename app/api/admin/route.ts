...
import { validateAdminPassword, createAdminClient } from '@/lib/admin-auth';
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const totalViews = stats?.reduce((sum, row) => sum + (row.view_count || 0), 0) || 0;
    const totalDownloads = stats?.reduce((sum, row) => sum + (row.download_count || 0), 0) || 0;
...
export async function GET_AUDIT(request: NextRequest) {
  ...
  try {
    ...
    if (search) {
      query = query.or(`id.ilike.%${search}%,admin_id.ilike.%${search}%,resource_id.ilike.%${search}%`);
    }
...
export async function GET_SESSIONS(request: NextRequest) {
  ...
  try {
    ...
    const sessionMap: Record<string, { created_at: string; last_active_at: string; strip_count: number; is_public: boolean }> = {};
