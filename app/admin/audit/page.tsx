...
import { FileText, User, Image as ImageIcon, StickerMule, Palette, Settings as SettingsIcon, Flag, Trash2, Clock, Search, Filter, MoreVertical, ChevronLeft, ChevronRight, Loader2, RefreshCw } from "lucide-react";
...
function generateMockAuditLogs(count: number): AuditLogEntry[] {
  const actions: AuditLogEntry['action'][] = ['create', 'update', 'delete', 'toggle', 'block', 'unblock', 'flag', 'unflag', 'feature', 'unfeature'];
  const resourceTypes: AuditLogEntry['resource_type'][] = ['template', 'sticker_pack', 'sticker', 'strip', 'session', 'settings', 'gallery'];
...
export default function AdminAuditPage() {
  ...
  const filteredLogs = logs.filter((log) => {
    if (searchQuery && !log.id.toLowerCase().includes(searchQuery.toLowerCase()) &&
  ...
  const getResourceIcon = (resource: string) => RESOURCE_ICONS[resource] || FileText;

  const getActionColor = (action: string) => {
    if (action.includes("delete") || action.includes("block") || action.includes("flag")) return "text-destructive bg-destructive/10";
    if (action.includes("create") || action.includes("feature") || action.includes("import")) return "text-emerald-600 bg-emerald-500/10";
    if (action.includes("update") || action.includes("toggle") || action.includes("un")) return "text-violet-600 bg-violet-500/10";
    ...
    <div className="space-y-6">
          ...
          <div className="bg-background rounded-2xl border border-border/80 shadow-sm overflow-hidden">
            <table className="w-full text-sm">
              <thead className="bg-secondary/30">
                <tr>
                  <th className="p-4 text-left font-semibold text-muted-foreground uppercase tracking-wide">Action</th>
                  <th className="p-4 text-left font-semibold text-muted-foreground uppercase tracking-wide">Resource</th>
                  <th className="p-4 text-left font-semibold text-muted-foreground uppercase tracking-wide">Admin</th>
                  <th className="p-4 text-left font-semibold text-muted-foreground uppercase tracking-wide">Details</th>
                  <th className="p-4 text-left font-semibold text-muted-foreground uppercase tracking-wide">When</th>
                  <th className="p-4 text-left font-semibold text-muted-foreground uppercase tracking-wide">&nbsp;</th>
                </tr>
              </thead>
              <tbody>
                {paginatedLogs.map((log) => {
                  ...
                  return (
                    <tr key={log.id} className="border-t border-border/50 hover:bg-muted/50 transition-colors">
                      <td className="p-4">
                      ...
                      <td className="p-4">
                        <div className="flex items-center gap-2">
                          <span className="p-2 rounded-lg bg-muted/50">
                            <ResourceIcon size={16} className="text-muted-foreground" />
                          </span>
                          <span className="font-semibold text-foreground">{log.resource_type.replace("_", " ")}</span>
                        </div>
                      </td>
                      <td className="p-4">
                        <p className="font-semibold text-foreground">{log.admin_id}</p>
                        <p className="text-xs text-muted-foreground">{log.ip_address}</p>
                      </td>
                      <td className="p-4">
                        <p className="font-semibold text-foreground truncate">{log.resource_id}</p>
                      </td>
                      <td className="p-4">
                        <p className="text-sm text-muted-foreground">{formatDate(log.created_at)}</p>
                      </td>
                      <td className="p-4">
                        <button onClick={() => setSelectedLog(log)} className="p-1.5 rounded-lg hover:bg-muted/50 text-muted-foreground">
      ...
      {selectedLog && (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4" onClick={() => setSelectedLog(null)}>
          <div className="bg-background rounded-3xl border border-border/80 shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            <div className="p-6 border-b border-border flex items-center justify-between">
              <h2 className="text-lg font-heading font-bold">Audit Entry Details</h2>
              <button onClick={() => setSelectedLog(null)} className="p-1.5 rounded-lg hover:bg-muted/50">
                <X size={18} />
              </button>
            </div>
            <div className="p-6 space-y-6">
              <div className="bg-secondary/30 rounded-xl p-4">
                <h3 className="text-sm font-semibold text-muted-foreground mb-3">Action</h3>
                <div className="flex items-center gap-3">
                  <span className={`p-3 rounded-xl ${getActionColor(selectedLog.action)}`}>
                    {getActionIcon(selectedLog.action)({ size: 24 })}
                  </span>
                  <div>
                    <p className="font-bold text-xl text-foreground">{selectedLog.action}</p>
                    <p className="text-sm text-muted-foreground">
                      {selectedLog.resource_type.replace("_", " ")} - {selectedLog.resource_id}
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div>
                  <p className="text-muted-foreground uppercase text-xs mb-1">Entry ID</p>
                  <p className="font-mono font-semibold">{selectedLog.id}</p>
                </div>
                <div>
                  <p className="text-muted-foreground uppercase text-xs mb-1">Timestamp</p>
                  <p className="font-semibold">{formatDate(selectedLog.created_at)}</p>
                </div>
                <div>
                  <p className="text-muted-foreground uppercase text-xs mb-1">Admin ID</p>
                  <p className="font-semibold">{selectedLog.admin_id}</p>
                </div>
                <div>
                  <p className="text-muted-foreground uppercase text-xs mb-1">IP Address</p>
                  <p className="font-mono">{selectedLog.ip_address}</p>
                </div>
                <div className="col-span-2">
                  <p className="text-muted-foreground uppercase text-xs mb-1">User Agent</p>
              ...
              {selectedLog.details && (
                <div className="bg-secondary/30 rounded-xl p-4">
                  ...
                  <pre className="text-xs text-muted-foreground overflow-x-auto">{JSON.stringify(selectedLog.details, null, 2)}</pre>
                </div>
              )}
            </div>
          </div>
      ...
      <div className="bg-amber-500/10 border border-amber-500/30 rounded-xl p-3 text-sm text-amber-600">
        <strong>Note:</strong> Audit logs will be pulled from database in production. Current view shows mock data.
      </div>
    </div>
  );
}
